...
export class CacheService {
  ...
  getCacheMetrics(): CacheMetrics {
    ...
    const hitRate =
      totalRequests > 0 ? (store.cacheHits / totalRequests) * 100 : 0;
  ...
  async cleanupStaleCache(): Promise<void> {
    ...
    if (metrics.staleCaches.length > 0) {
      console.log(
        `🧹 Cleaning up ${metrics.staleCaches.length} stale cache entries`
      );
  ...
  async optimizeCache(): Promise<void> {
    ...
    if (metrics.cacheSize > 500) {
      console.log(
        "💾 Large cache detected, consider implementing LRU eviction"
      );
    }
  }
  ...
  getCacheRecommendations(): string[] {
    ...
    if (metrics.staleCaches.length > 3) {
      recommendations.push(
        "Multiple stale caches detected, increase refresh frequency"
      );
    }
    ...
    if (metrics.totalRequests < 10) {
      recommendations.push(
        "Low request count, cache warming may be beneficial"
      );
    }
